...
```bash
sam-video-sync-test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-video-sync-test$ sam local start-api
sam-video-sync-test$ curl http://localhost:3000/
```
...
```bash
sam-video-sync-test$ sam logs -n HelloWorldFunction --stack-name sam-video-sync-test --tail
```
...
```bash
sam-video-sync-test$ cd hello-world
...
```bash
sam delete --stack-name sam-video-sync-test
```
